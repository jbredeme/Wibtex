...
# Construct labels
input_doc_label = Label(root, text="Input Document",
                        fg='white', bg="#2c5897", font=("Calibri", 14))

input_bib_label = Label(root, text="Input Database",
                        fg='white', bg="#2c5897", font=("Calibri", 14))

input_style_label = Label(root, text="Input Style",
                          fg='white', bg="#2c5897", font=("Calibri", 14))

output_doc_label = Label(root, text="Output Document",
                         fg='white', bg="#2c5897", font=("Calibri", 14))
...
def run(event, arg):
    ...
    if not input_doc_entry.get().endswith('.docx'):
        ...
        showinfo("Window", "Expected Word file extension \
                 '.docx' for first argument")
        return
    ...
    if not input_bib_entry.get().endswith('.bib'):
        ...
        showinfo("Window", "Expected BibTeX file extension \
                 '.bib' for second argument")
        return
    ...
    main.execute(input_bib_entry.get(), input_doc_entry.get(),
                 initial.get(), output_doc_entry.get(), log)
    showinfo("Window", "Execution is complete! :)")
    return

# Construct buttons
input_doc_button = Button(root, text="Browse",
                          bg="grey", fg="white", font=("Calibri", 10))
input_bib_button = Button(root, text="Browse",
                          bg="grey", fg="white", font=("Calibri", 10))
execute_button = Button(root, text="Run",
                        bg="grey", fg="white", font=("Calibri", 10))

input_doc_button.bind("<Button-1>", lambda event,
                      arg=input_doc_entry: browse_path(event, arg))
input_bib_button.bind("<Button-1>", lambda event,
                      arg=input_bib_entry: browse_path(event, arg))
